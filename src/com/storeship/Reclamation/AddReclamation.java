...
String imageFilePath = ""; // Variable to store the selected image file path
    ...
    public AddReclamation(Detail_Commande c) {
...
inputContainer.add(addImageButton);
    ...
    if (buttonGroup.isSelected()) {
        ...
        if (hasEnoughWords) {
            ...
            Reclamation reclamation = new Reclamation(0, SessionManager.getUser().getId(), c.getCommande().getId(), "pending",image, new Date(), contenu, c.getProduit().getId(), 0, type);
            // Call the service to add the new reclamation
            System.out.println("Reclamation to send --------------------" + reclamation);
